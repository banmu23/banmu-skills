...
2. 如果用户给的是飞书文档链接，默认先读取原文，再新建美化版；只有用户明确说“直接改原文档”时，才更新原文档。
   - 读取和修改已有文档时，以在线最新版本为准，保留人工修改，优先局部调整，不擅自移动文档目录。
...
2. 根据用途选择风格，不套固定模板。
3. 重构标题、摘要、目录、章节层级和阅读路径。长课程、指南、SOP、方案和交付文档通常先收束为 3～7 个并列一级模块，再按真实内容展开二级或三级；短文不为凑目录强拆层级。同级标题保持并列，使用飞书自动编号时不要在标题文字里重复编号。
4. 用 callout、分栏、表格、检查清单、时间线、流程图/画板等结构块提升可读性。
5. 保证开头结论前置，连续纯文本不超过 3 段，每个主要章节至少有 1 个非纯文本结构块。
6. 面向客户或学员的实操文档只保留首版成功真正必要的动作、检查和提醒；不要用“至少完成 X 项”“全部都要准备”制造无效负担，能准确表达时优先写“第一版先完成这些就够了”或“任选一种方法即可”。
